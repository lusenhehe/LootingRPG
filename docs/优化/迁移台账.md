...
| MIG-005 | player | `src/logic/playerStats.ts` | `src/domains/player/services/recalculatePlayerStats.ts` | 已切换 | Agent | 2026-03-18（计划） | build + check:orphan | 应用内调用已切换，保留兼容导出待 Phase 3 删除 |
...
| MIG-008 | map | `src/logic/mapProgress.ts` | `src/domains/map/services/progress.ts` | 已切换 | Agent | 2026-03-28（计划） | build + check:orphan | 应用内调用已切换，保留兼容导出待 Phase 3 删除 |
...
| MIG-027 | inventory | `src/logic/inventory/actions.ts` | `src/domains/inventory/services/actions.ts` | 已切换 | Agent | 2026-03-22（计划） | build + check:orphan | 应用内调用已切换，保留兼容导出待 Phase 3 删除 |
| MIG-028 | inventory | `src/logic/inventory/autoSell.ts` | `src/domains/inventory/services/autoSell.ts` | 已切换 | Agent | 2026-03-22（计划） | build + check:orphan | 应用内调用已切换，保留兼容导出待 Phase 3 删除 |
| MIG-029 | inventory | `src/logic/inventory.ts` | `src/domains/inventory/services/quickSell.ts` | 已切换 | Agent | 2026-03-22（计划） | build + check:orphan | 应用内调用已切换，保留兼容导出待 Phase 3 删除 |
...
| MIG-033 | battle | `src/logic/battle/index.ts` | `src/domains/battle/services/session.ts` | 迁移中 | Agent | 2026-03-31（计划） | build + check:orphan | 已建立 domains 实现，旧入口改兼容导出 |
...
| MIG-038 | app | `src/logic/gameState/index.ts` | `src/app/state/index.ts` | 已切换 | Agent | 2026-03-29（计划） | build + check:orphan | 应用内调用已切换，保留兼容导出待 Phase 3 删除 |
...
| MIG-040 | infra | `src/logic/i18n/labels.ts` | `src/infra/i18n/labels.ts` | 已切换 | Agent | 2026-03-29（计划） | build + check:orphan | 应用内调用已切换，保留兼容导出待 Phase 3 删除 |
...
| MIG-043 | player | `src/logic/uiHelpers.ts` | `src/domains/player/services/derivedStats.ts` | 已切换 | Agent | 2026-03-29（计划） | build + check:orphan | 新建 domains 服务，PlayerPanel 已切换，旧入口保留兼容导出 |
| MIG-044 | battle | `src/logic/stats/monsterScaling.ts`（调用侧） | `src/domains/battle/services/monsterScaling.ts` | 已切换 | Agent | 2026-03-29（计划） | build + check:orphan | `domains/monster/config.ts` 已改 domains 直连 |
| MIG-045 | battle/app | `src/logic/battle/index.ts`（调用侧） | `src/domains/battle/services/session.ts` | 已切换 | Agent | 2026-03-31（计划） | build + check:orphan | `App.tsx` 已切换 domains 直连，旧入口保留兼容导出 |
