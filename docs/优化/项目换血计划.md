...
### 到 Phase 2 预计还需（滚动更新）
- 预计 **3~5 次对话**（每次 >=5 文件迁移）可进入 Phase 2 启动条件。
- 关键前置：完成 battle 主链路迁移、monster stats/scaling 迁移、gameState 目录收敛。

### Phase 1 剩余待迁移文件清单（滚动更新）
- `src/logic/battle/index.ts` -> `src/domains/battle/services/*`
- `src/logic/gameState/*`（`globalState/index/stateManager/inventoryState`）-> `src/app` + `src/domains/*/state`
- `src/logic/i18n/labels.ts` -> `src/infra/i18n/labels.ts`
- `src/logic/stats/monsterStats.ts`、`src/logic/stats/monsterScaling.ts` -> `src/domains/battle/services/*`
- `src/config/game/equipment.ts`、`src/config/game/progression.ts`（按需拆分到 domain config）
