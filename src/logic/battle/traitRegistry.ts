...
registerTrait('thorns', {
  onDamaged: (ctx, payload) => {
    ...
    ctx.note(i18n.t('trait.note.thorns', { damage: reflectedDamage }));
  },
});
...
registerTrait('lifesteal', {
  onDamaged: (ctx, payload) => {
    ...
    ctx.note(i18n.t('trait.note.lifesteal_enemy', { value: healed }));
  },
});
...
registerTrait('double_attack', {
  onAttack: (ctx, payload) => {
    ...
    if (Math.random() < strikeChance) {
      ...
      ctx.note(i18n.t('trait.note.double_attack'));
    }
  },
});
...
registerTrait('shield_on_start', {
  onBattleStart: (ctx) => {
    ...
    ctx.note(i18n.t('trait.note.shield_on_start'));
  },
});
...
registerTrait('rage_on_low_hp', {
  onLowHp: (ctx, payload) => {
    ...
    ctx.note(i18n.t('trait.note.rage_on_low_hp'));
  },
});
